...
void FNDIIOPluginModule::StartupModule()
{
	...
	else
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("NDIIO Plugin: Detected Android platform. Verifying NDI integration."));
...
#elif PLATFORM_ANDROID
	// Log fake handle for sanity check
	
	UE_LOG(LogTemp, Log, TEXT("NDIIO Plugin: Assigned dummy NDI_LIB_HANDLE on Android."));
	return true;
#endif
}
...
#if (PLATFORM_LINUX || PLATFORM_LINUXARM64 || PLATFORM_ANDROID)
#if PLATFORM_ANDROID
	// Android: NDIlib_destroy still needed
	NDIlib_destroy();
	this->NDI_LIB_HANDLE = nullptr;
#endif
#endif
